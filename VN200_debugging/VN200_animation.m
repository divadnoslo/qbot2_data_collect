...
for t = 0:dT:nSec               % Retrieve IMU data from VN200
    ...
    k = ??;            % Normalized axis of rotation (dimless)
    theta = ??;         % Angle of rotation (rad)

    K = [  ??;    % Skew-symmetric matrix form of the normalized axis of rotation
         ??); 
        ??];
   
    delta_C = ??; % Rodrigues formula
    
    C =  ??;           % Incrementally update the orientation matrix
